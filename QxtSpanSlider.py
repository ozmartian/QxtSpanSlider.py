...
from PyQt5.QtCore import QRect, QRectF, QPoint, Qt, pyqtProperty, pyqtSignal, pyqtSlot
from PyQt5.QtGui import QColor, QPen, QPalette, QLinearGradient
from PyQt5.QtWidgets import QAbstractSlider, QSlider, QStyle, QStylePainter, QStyleOptionSlider, QStyleFactory
...
class QxtSpanSlider(QSlider):
    NoHandle, FreeMovement = None, None
    LowerHandle, NoCrossing = 1, 1
    UpperHandle, NoOverlapping = 2, 2
    ...
    def pick(self, p):
        return p.x() if self.orientation() == Qt.Horizontal else p.y()
    ...
    def drawSpan(self, painter, rect):
        ...
        if opt.orientation == Qt.Horizontal:
            self.setupPainter(painter, opt.orientation, groove.center().x(), groove.top(), groove.center().x(),
                              groove.bottom())
        else:
            self.setupPainter(painter, opt.orientation, groove.left(), groove.center().y(), groove.right(),
                              groove.center().y())
        # draw groove
